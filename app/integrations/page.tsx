...
const handleConnectSalesforce = async (role: string, env: 'production' | 'sandbox' = 'production') => {
  console.log("[3-KEY-LOCK][UI] Initiating Salesforce connect", { role, env });
  const token = await getToken();
  if (!token) return;
  await connectSalesforce(token, role, env);
};
