  ...
  return {
    ...
    generatedAt:
      ...
      ("generatedAt" in (evidence.causalProof || {})
        ...
        : undefined),
    liveDiagnostic: evidence.liveDiagnostic,
    astTransformation,
    blastRadius,
...
function normalizeCausalProof(
  ...
  if (
    statisticallyIdentifiable &&
    (sandboxPassed || sandboxStatus.length === 0)
  ) {
    status = "PROVEN";
...
function normalizeNodeType(value: string): BlastGraphNode["type"] {
  ...
  if (normalized.includes("apex") || normalized.includes("class"))
    return "Apex";
  if (normalized.includes("flow") || normalized.includes("trigger"))
    return "Flow";
